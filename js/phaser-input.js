    ...
    var InputElement = (function() {
        function InputElement(game, id, type, value) {
            ...
            if (!bedbugGameCore.game.device.desktop)
                this.element.style.top = (40).toString() + 'px';
            else
                this.element.style.top = (-40).toString() + 'px';
            this.element.style["margin-left"] = "45%";
            this.element.style["z-index"] = "999";
            this.element.style["text-align"] = "center"
            // this.element.style.left = (0).toString() + 'px';
        ...
        InputElement.prototype.focus = function() {
            ...
            bedbugGameCore.isDisplayingInput = true;

            this.element.focus();

            if (!bedbugGameCore.isDesktop()) {
                setTimeout(function() {
                    bedbugGameCore.inputBlock.alpha = 1;
                    bedbugGameCore.inputBlock.inputEnabled = true;
                    if (!_this.game.device.iOS) { // expand the game in order to be clickable
                        bedbugGameCore.game.scale.scaleMode = Phaser.ScaleManager.EXACT_FIT;
                    }
        ...
        InputElement.prototype.blur = function() {
            ...
            this.element.style.display = "none";
            this.element.style.opacity = 0;
            //  console.log("Setting Displaying Input to False");
            // bedbugGameCore.isDisplayingInput = false;
        };
...
(function(Fabrique) {
    var InputField = (function(_super) {
        ...
        function InputField(game, x, y, inputOptions) {
            ...
            this.inputOptions.defaultValue = inputOptions.value || "0";
            ...
            this.domElement.setMax(this.inputOptions.max || 99999, this.inputOptions.min || 0);
        ...
        InputField.prototype.checkDown = function(e) {
            ...
            if (this.input.checkPointerOver(e)) {
                ...
                // Due to IE bug with carret. Every time the user clicks on input we empty the textfield
                this.resetText();
                this.startFocus();
            }
            else {
        ...
        InputField.prototype.endFocus = function() {
            ...
            if (this.focus) {
                ...
                if (this.value == undefined || this.value == "") {
                    this.value = this.inputOptions.defaultValue;
                    this.setText(this.value);
                }
                //  console.log(this.value+" | "+ this.inputOptions.defaultValue);
                if (bedbugGameCore.inputBlock) {
                    bedbugGameCore.inputBlock.alpha = 0;
                    bedbugGameCore.inputBlock.inputEnabled = false;
                    bedbugGameCore.game.scale.scaleMode = Phaser.ScaleManager.SHOW_ALL;
                    setTimeout(function() {
                        // $("canvas").css("margin-top", "0").css("margin-bottom", "0");

                        if (bedbugGameCore.game.device.iPhone)
                ...
                //     // this.placeHolder.visible = true;
                //     console.log("Value is not permited");
                //     this.value = 0;
                //     this.domElement.value = this.value;
                //     this.updateText();
                // }
        ...
        InputField.prototype.updateText = function() {
            ...
            if (this.inputOptions.type === Fabrique.InputType.password) {
                ...
                if (val < parseFloat(this.inputOptions.min)) {
                    text = this.value = this.domElement.value = this.inputOptions.min;
                }
                else if (val > parseFloat(this.inputOptions.max)) {
        ...
        InputField.prototype.setCaretOnclick = function(e) {
            ...
            // this.domElement.setCaretPosition(0);
