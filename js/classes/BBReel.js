var BBReel = function(game, x, y, name, id, parent, addtostage) {
    ...
    if (Timmings.real_delay)
    //  FIX: Setting the spinTime as the initSpinTime and adding one more loop to each succesive reel may fix the slowing down of reels in order to make them stagger stop
        ...
        this.spinTime = bedbugGameCore.settings.fast_spins ? this.initSpinTime + (parseInt(this.id) * Timmings.reel_fast_spin_time_space) : this.initSpinTime + (parseInt(that.id) * Timmings.reel_time_space);
...
BBReel.prototype.Spin = function(results, delay, ReelPlayType, anticipationIndex, onSpinFinished) {
    ...
    if (Timmings.real_delay)
        this.numOfSpins = bedbugGameCore.settings.fast_spins ? Timmings.reel_fast_spins_num || 1 : (Timmings.reel_spins_num || 5) + (2 - (Timmings.reel_spins_num - 1 - this.id)); //  FIX: this will result in each succesive reel making one more loop [(Timmings.reel_spins_num || 5) - (5 -this.id)];
    else
        this.numOfSpins = bedbugGameCore.settings.fast_spins ? Timmings.reel_fast_spins_num || 1 : (Timmings.reel_spins_num || 5); //  FIX: this will result in each succesive reel making one more loop [(Timmings.reel_spins_num || 5) - (5 -this.id)];
    ...
    spinLoop.onStart.add(function() {
        ...
        setTimeout(function() {
            ...
            if (
                ...
                bedbugGameCore.settings.fast_spins != true) {
    ...
    spinLoop.onRepeat.add(function() {
        ...
        if (bedbugGameCore.stopSpin && bedbugGameCore.stop_spin_index == that.id) {
            ...
            spinLoop.updateTweenData("duration", that.spinTime);
    ...
    spinLoop.onComplete.add(function() {
        if ((that.ReelPlayType == 3 || that.ReelPlayType == 4) && !bedbugGameCore.stopSpin)
            that.position.y = that.startY;
    });
    ...
    anticipation.onComplete.add(function() {
        if (that.ReelPlayType == 4) {
            ...
            that.scatters_appeard = bedbugGameCore.scatters_appeard;
        }
    ...
    spinlast.onComplete.add(function() {
        setTimeout(function() {
            ...
            if (that.ReelPlayType == 2 || that.ReelPlayType == 4) {
                // Show scatter event if it has a chance to succeed
                if (!(that.id == 3 && that.scatters_appeard < 2) && !(that.id == 4 && that.scatters_appeard < 3)) {
...
BBReel.prototype.randomizeSlots = function(startingIndex, times, useThisSymbol) {
    ...
    _.times(times, function(k) {
        ...
        if (startingIndex == 0 && index < 3) {
            var slotDrop = that.game.add.tween(that.Slots[index].position).from({
                y: -600
            }, 500, "Sine.easeOut", true, 600 + (that.id * 150) + (index * 100), 0, false);
