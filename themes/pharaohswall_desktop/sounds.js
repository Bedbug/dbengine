Sounds = {
    ...
    create: function(context) {
        ...
        bedbugEventsSystem.addListener('ON_STONE_DROP', ON_STONE_DROP);

        function ON_STONE_DROP() {
            context.game.sound.play('stone_drop', bedbugGameCore.sound_effects_volume);
        };
        
        bedbugEventsSystem.addListener('ON_POINTS_WIN', ON_POINTS_WIN);

        function ON_POINTS_WIN() {
            context.game.sound.play('win_points', bedbugGameCore.sound_effects_volume);
        };
        ...
        function ON_AMBIANCE_START_NEW() {
            ...
            if (bedbugGameCore.ambiance_volume > 0)
                if (context.game.cache.isSoundDecoded(bedbugGameCore.ambianceKey))
                    ambiance.fadeIn(5000, true);
                else
                    ambiance.play("", 0, bedbugGameCore.ambiance_volume, true);
            else
